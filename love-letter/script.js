...
/* ---------- Scene 6: the paper grows into the full letter (FLIP morph) ---------- */
function expandLetter() {
  const paper = document.getElementById('envLetterPaper');
  const envelope = document.getElementById('envelope');
  const hint = document.getElementById('envHint');

  // FIRST: capture the paper's current on-screen box, still tiny and tucked in the envelope
  const first = paper.getBoundingClientRect();

  // LAST: flip on the class that makes it the full-size centered letter card,
  // clear the leftover rise-transform (it would otherwise contaminate this
  // measurement), then immediately measure where that puts it
  ...
  // INVERT: pin it back to where it visually was, with zero transition
  paper.style.transition = 'none';
  paper.style.transform = `translate(${dx}px, ${dy}px) scale(${sx}, ${sy})`;
  paper.style.borderRadius = '4px';
  void paper.offsetWidth;

  // PLAY: release it — the browser animates the transform back to identity
  requestAnimationFrame(() => {
    paper.style.transition = 'transform 1.3s cubic-bezier(.22,.85,.3,1), border-radius 1s ease';
    paper.style.transform = 'translate(0,0) scale(1,1)';
    paper.style.borderRadius = '18px';
  });

  // fade the envelope shell out from underneath as the paper grows past it
